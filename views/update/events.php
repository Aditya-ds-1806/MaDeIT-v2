...
<body class="p-2">
        ...
        <form id="form" class="text-left my-5">
            <div class="input-group">
                <div class="custom-file">
                    <input type="file" class="custom-file-input" accept=".pdf,.jpg,.jpeg,.png" name="file" id="inputGroupFile02" onchange="$(this).next('.custom-file-label').html($(this).val()); $('#name').val($(this).val())" required>
                    <label id="fileName" class="custom-file-label" for="inputGroupFile02" aria-describedby="inputGroupFileAddon02">Upload brochure or image associated with event</label>
                </div>
    ...
    <script>
        $("#form").on('submit', function(e) {
            ...
            if ($("input[type = 'file']").val().split('.').pop() === 'pdf') formData.append("target-dir", "/brochures/");
            else formData.append("target-dir", "/images/events/");
            $.ajax({
                type: "POST",
