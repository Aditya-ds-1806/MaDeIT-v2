...
<body class="p-2">
        ...
        <form id="form" class="text-left my-5">
            <div class="input-group">
                <div class="custom-file">
                    ...
                    <label id="fileName" class="custom-file-label" for="inputGroupFile02" aria-describedby="inputGroupFileAddon02">Upload logo of programme</label>
                </div>
    ...
    <script>
        $("#form").on('submit', function(e) {
            ...
            formData.append("target-dir", "/images/programmes/");
            $.ajax({
                type: "POST",
    ...
    <script>
        (async function() {
            ...
            const res = await fetch('public/json/programs.json');
            ...
            const editor = new JSONEditor(container, {
                ...
                schema: {
                    ...
                    items: {
                        ...
                        minProperties: Object.keys(programs[0]).length,
                        maxProperties: Object.keys(programs[0]).length,
                        required: Object.keys(programs[0])
                    }
                },
                onValidate: function(programs) {
                    var errors = [];
                    for (let i = 0; i < programs.length; i++) {
                        var program = programs[i];
                        Object.values(program).forEach(val => {
                            if (val.trim() === "") {
                                errors.push({
                                    path: [i],
                                    message: `"${Object.keys(program)[Object.values(program).indexOf(val)]}" field cannot be blank.`
                                });
                            }
                        })
                    }
